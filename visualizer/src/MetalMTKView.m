...
- (void)keyDown:(NSEvent *)event {
  ...
  case (KEY_G): {
    ...
    if (mesh_data->width <= mesh_data->height) {
      ...
      mesh_data->raytracing_divs_y =
          10 * mesh_data->height / (float)(mesh_data->width);
    } else {
      mesh_data->raytracing_divs_x =
          10 * mesh_data->width / (float)(mesh_data->height);
  ...
  case (KEY_T): {
    ...
    NSPoint screenTouchPoint = NSEvent.mouseLocation;
    NSWindow *window = event.window;
    NSPoint touchPoint = {.x = screenTouchPoint.x - window.frame.origin.x,
                          .y = screenTouchPoint.y - window.frame.origin.y};
