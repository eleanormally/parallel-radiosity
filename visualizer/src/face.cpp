...
Vec3f Face::RandomPoint() const {
  ...
  Vec3f answer =
      s * t * a + s * (1 - t) * b + (1 - s) * t * d + (1 - s) * (1 - t) * c;
  return answer;
}
...
bool Face::intersect(const Ray& r, Hit& h, bool intersect_backfacing) const {
  ...
  return triangle_intersect(r, h, a, b, c, intersect_backfacing) ||
         triangle_intersect(r, h, a, c, d, intersect_backfacing);
}

bool Face::triangle_intersect(const Ray& r, Hit& h, Vertex* a, Vertex* b,
                              Vertex* c, bool intersect_backfacing) const {
  ...
  float detA = Matrix::det3x3(
      a->get().x() - b->get().x(), a->get().x() - c->get().x(), Rd.x(),
  ...
  float beta = Matrix::det3x3(
                   a->get().x() - Ro.x(), a->get().x() - c->get().x(), Rd.x(),
                   a->get().y() - Ro.y(), a->get().y() - c->get().y(), Rd.y(),
                   a->get().z() - Ro.z(), a->get().z() - c->get().z(), Rd.z()) /
               detA;
  float gamma =
      Matrix::det3x3(a->get().x() - b->get().x(), a->get().x() - Ro.x(), Rd.x(),
                     a->get().y() - b->get().y(), a->get().y() - Ro.y(), Rd.y(),
                     a->get().z() - b->get().z(), a->get().z() - Ro.z(),
                     Rd.z()) /
      detA;

  if (beta >= -0.00001 && beta <= 1.00001 && gamma >= -0.00001 &&
      gamma <= 1.00001 && beta + gamma <= 1.00001) {
    h = h2;
...
bool Face::plane_intersect(const Ray& r, Hit& h,
                           bool intersect_backfacing) const {
...
std::unordered_map<const Face*, const Vec3f> normalCache;
Vec3f Face::computeNormal() const {
  const auto cachedNormal = normalCache.find(this);
  if (cachedNormal != normalCache.end()) {
    return cachedNormal->second;
  }
  ...
  Vec3f c = (*this)[2]->get();
  Vec3f d = (*this)[3]->get();
  Vec3f normal = (ComputeNormal(a, b, c) + ComputeNormal(a, c, d));
  normal.Normalize();
  normalCache.insert(std::make_pair(this, normal));
  return normal;
}
