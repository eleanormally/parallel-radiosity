...
class RayTree {
 ...
 public:
  ...
  static int  getIdx() { return idx; }
  static void incrementIdx() { idx++; }
  ...
  static int numSegments() {
    return main_segments.size() + shadow_segments.size() +
           reflected_segments.size() + transmitted_segments.size();
  }
  static int  triCount();
