...
void RayTree::packMesh(float*& current) {
  ...
  for (i = 0; i < main_segments.size(); i++) {
    addBox(current, main_segments[i].getStart(), main_segments[i].getEnd(),
           main_color, width);
  }
  for (i = 0; i < shadow_segments.size(); i++) {
    addBox(current, shadow_segments[i].getStart(), shadow_segments[i].getEnd(),
           shadow_color, width);
  }
  for (i = 0; i < reflected_segments.size(); i++) {
    addBox(current, reflected_segments[i].getStart(),
           reflected_segments[i].getEnd(), reflected_color, width);
  }
  for (i = 0; i < transmitted_segments.size(); i++) {
    addBox(current, transmitted_segments[i].getStart(),
           transmitted_segments[i].getEnd(), transmitted_color, width);
  }
}
