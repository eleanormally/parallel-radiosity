...
class Face {
 ...
 public:
  ...
  Vertex* operator[](int i) const {
    ...
    if (i == 1)
      return edge->getNext()->getStartVertex();
    if (i == 2)
      return edge->getNext()->getNext()->getStartVertex();
    if (i == 3)
      return edge->getNext()->getNext()->getNext()->getStartVertex();
    assert(0);
    exit(0);
  }
  Edge* getEdge() const {
  ...
  Vec3f computeCentroid() const {
    return 0.25f * ((*this)[0]->get() + (*this)[1]->get() + (*this)[2]->get() +
                    (*this)[3]->get());
  }
 ...
 protected:
  // helper functions
  bool triangle_intersect(const Ray& r, Hit& h, Vertex* a, Vertex* b, Vertex* c,
