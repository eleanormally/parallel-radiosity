...
enum FACE_TYPE {
  FACE_TYPE_ORIGINAL,
  FACE_TYPE_RASTERIZED,
  FACE_TYPE_SUBDIVIDED
};
...
class Mesh {
 ...
 public:
  ...
  int numRasterizedPrimitiveFaces() const {
    return rasterized_primitive_faces.size();
  }
  ...
  int numFaces() const {
    return subdivided_quads.size() + rasterized_primitive_faces.size();
  }
  Face* getFace(int i) const {
    int num_faces = numFaces();
    assert(i >= 0 && i < num_faces);
    if (i < (int)subdivided_quads.size())
      return subdivided_quads[i];
    else
  ...
  void addRasterizedPrimitiveFace(Vertex* a, Vertex* b, Vertex* c, Vertex* d,
                                  Material* material) {
    addFace(a, b, c, d, material, FACE_TYPE_RASTERIZED);
  }
  void addOriginalQuad(Vertex* a, Vertex* b, Vertex* c, Vertex* d,
                       Material* material) {
    addFace(a, b, c, d, material, FACE_TYPE_ORIGINAL);
  }
  void addSubdividedQuad(Vertex* a, Vertex* b, Vertex* c, Vertex* d,
                         Material* material) {
 ...
 private:
  ...
  void addFace(Vertex* a, Vertex* b, Vertex* c, Vertex* d, Material* material,
